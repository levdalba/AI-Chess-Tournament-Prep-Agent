...
class ChessDataFetcher:
    ...
    async def fetch_multiple_platforms(self, requests: List[FetchRequest]) -> List[FetchResult]:
    ...
    async def fetch_all_platforms(self, 
                                ...
                                **kwargs) -> Dict[Platform, FetchResult]:
    ...
    async def get_player_info_all_platforms(self, username: str) -> Dict[Platform, Dict[str, Any]]:
    ...
    async def fetch_player_games(self, username: str, platform: str, limit: int = 100) -> List[Game]:
        ...
        request = FetchRequest(username=username, platform=platform_enum, max_games=limit)
            ...
            while True:
                ...
                metadata = GameMetadata(
                    ...
                    white=Player(
                        ...
                        rating=int(headers.get("WhiteElo", "0")) if headers.get("WhiteElo", "0").isdigit() else None,
                        platform=platform
                    ),
                    black=Player(
                        name=headers.get("Black", "Unknown"),
                        rating=int(headers.get("BlackElo", "0")) if headers.get("BlackElo", "0").isdigit() else None,
                        platform=platform
                    ),
    ...
    def combine_pgn_results(self, results: Dict[Platform, FetchResult]) -> FetchResult:
        ...
        for platform, result in results.items():
            if result.success and result.pgn_content:
                all_pgns.append(f"# Games from {platform.value}\n\n{result.pgn_content}")
                total_games += result.games_count
                successful_platforms.append(platform)
                all_metadata[f"{platform.value}_metadata"] = result.metadata
            
            if result.errors:
                all_errors.extend([f"{platform.value}: {error}" for error in result.errors])
