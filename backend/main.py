...
async def search_player(
    ...
    try:
        ...
        player = Player(
            ...
            rating=first_game.white_rating if first_game.white_player.lower() == request.username.lower() else first_game.black_rating,
...
async def analyze_player(
    ...
    try:
        ...
        player = Player(
            ...
            platform=request.platform,
            rating=games[0].white_rating if games[0].white_player.lower() == request.player_username.lower() else games[0].black_rating
        )
        ...
        if request.include_openings:
            repertoire = opening_analyzer.analyze_player_openings(games_data, request.player_username)
            opening_repertoire = {
                'as_white': [
                    {
                        'eco': v.eco,
        ...
        if request.include_weaknesses and chess_analyzer:
            try:
                async with chess_analyzer as analyzer:
                    ...
                    pgn_strings = [game.pgn for game in games[:10]]  # Limit to avoid timeout
                    analyses = await analyzer.analyze_multiple_games(pgn_strings)
                    if analyses:
                        weaknesses = analyzer.get_weakness_patterns(analyses, request.player_username)
            except Exception as e:
                logger.error(f"Error in weakness analysis: {e}")
        
        # Game statistics
        total_games = len(games)
        wins = sum(1 for g in games if (g.white_player.lower() == request.player_username.lower() and g.result == "1-0") or 
                                      (g.black_player.lower() == request.player_username.lower() and g.result == "0-1"))
...
async def generate_prep_plan(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Prep plan generation error: {str(e)}")
